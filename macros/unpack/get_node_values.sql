...
{%- macro default__get_node_values() -%}
    ...
    {%- for node in nodes_list -%}
        ...
        {%- set contract = node.contract.enforced if node.contract else 0 -%}
        ...
        {%- set values_line  = 
            [
                ...
                "cast(" ~ convert_boolean_value(node.config.enabled | trim) ~ " as " ~ dbt.type_boolean() ~ ")",
                ...
                "cast(" ~ convert_boolean_value(contract | trim)  ~ " as " ~ dbt.type_boolean() ~ ")",
                ...
                "cast(" ~ convert_boolean_value(dbt_project_evaluator.is_not_empty_string(node.test_metadata) | trim) ~ " as " ~ dbt.type_boolean() ~ ")",
                "cast(" ~ convert_boolean_value(exclude_node) ~ " as " ~ dbt.type_boolean() ~ ")",
            ]
        %}
